...
const GAME_INSTRUCTIONS = [
  ...
  },
  {
    ...
    content:
      "Dùng CHUỘT click vào một viên kim cương, sau đó click vào viên cạnh bên (trên, dưới, trái, phải) để hoán đổi vị trí. Lưu ý: Chỉ có thể hoán đổi nếu nước đi đó tạo ra ít nhất một hàng hoặc cột có 3 viên cùng màu.",
  },
  {
  ...
  },
  {
    ...
    content:
      "Nếu sau khi nổ, các viên kim cương mới rơi xuống tự động tạo thành các hàng/cột trùng khớp, bạn sẽ nhận được Combo. Điểm số sẽ được nhân (x2, x3, x4...) theo số tầng Combo liên tiếp.",
  },
  {
    title: "Cơ chế 'Máy tự động'",
    content:
      "Để tăng độ khó, cứ mỗi 10 giây nếu bạn không thực hiện nước đi, Máy sẽ tự động thực hiện một lần hoán đổi ngẫu nhiên. Hãy nhanh tay để không bị Máy làm xáo trộn chiến thuật của bạn!",
  },
  {
    title: "Quyền trợ giúp (Hint)",
    content:
      "Nếu không tìm thấy nước đi, hãy nhấn nút HINT. Một cặp kim cương có thể tạo ra tổ hợp sẽ nhấp nháy. Tuy nhiên, mỗi lần dùng bạn sẽ bị trừ một lượng điểm nhỏ.",
  },
];
...
export default function Match3Game({ setControllerConfig }) {
  ...
  const processMatches = useCallback(
    async (boardState, isPlayerMove = true, multiplier = 1) => {
  ...
  const handleCellClick = useCallback(
    async (row, col) => {
      ...
      if (selectedPos === null) {
        ...
        // Try to swap
        const isAdjacent =
          (Math.abs(selectedPos.row - row) === 1 && selectedPos.col === col) || (Math.abs(selectedPos.col - col) === 1 && selectedPos.row === row);
    ...
    return (
      <div className="flex flex-col items-center gap-6 w-full max-w-2xl" ref={containerRef}>
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-sm">Click để chọn, click ô kề để swap</p>
        </div>
        ...
        <div className="flex gap-4 items-center">
          {times.map((t, idx) => (
            ...
            >
              <Clock className={cn("w-8 h-8", selectedTimeOption === idx ? "text-violet-400" : "text-muted-foreground")} />
              <span className={cn("text-2xl font-black", selectedTimeOption === idx ? "text-violet-400" : "text-muted-foreground")}>{t} phút</span>
            </button>
          ))}
        </div>
        ...
        <AlertDialog open={showInstructions} onOpenChange={setShowInstructions}>
          <AlertDialogContent className="bg-background border-border">
            ...
            <AlertDialogFooter>
              <AlertDialogAction className="bg-violet-600 hover:bg-violet-500">Đã hiểu</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <AlertDialog open={showExitDialog} onOpenChange={setShowExitDialog}>
          <AlertDialogContent className="bg-background border-border">
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={handleExitWithoutSave} className="bg-card text-foreground hover:bg-muted">
                Không lưu
              </AlertDialogCancel>
    ...
    <div className="flex flex-col items-center w-full h-full px-4 py-4 overflow-y-auto">
      <div className="flex flex-col items-center gap-3 w-full max-w-5xl">
        ...
        <div className="w-full max-w-2xl grid grid-cols-2 gap-4 flex-shrink-0">
          <ScoreBar label="Điểm số của tôi" score={playerScore} target={targetScore} icon={<User className="w-4 h-4" />} color="emerald" />
          <ScoreBar label="Điểm số của máy" score={computerScore} target={targetScore} icon={<Cpu className="w-4 h-4" />} color="rose" />
        </div>
        ...
        <div className="relative flex-shrink-0">
          ...
          <div
            className={cn(
              "relative grid gap-1 p-3 bg-card rounded-2xl border-4 shadow-2xl ring-1 ring-white/5 transition-all",
            ...
            {gameStatus === "win" && (
              <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md flex flex-col items-center justify-center rounded-xl z-50 animate-in fade-in zoom-in">
                <div className="p-8 bg-gradient-to-br from-emerald-900/90 to-teal-900/90 border border-emerald-500/50 rounded-3xl shadow-2xl flex flex-col items-center">
                  ...
                  <p className="text-emerald-300 text-sm mb-6 font-mono">
                    SCORE: {playerScore} | TIME: {elapsedTime}s
                  </p>
                  <Button
                    onClick={() => {
                      setGameStarted(false);
            ...
            {gameStatus === "lose" && (
              <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md flex flex-col items-center justify-center rounded-xl z-50 animate-in fade-in zoom-in">
                <div className="p-8 bg-gradient-to-br from-rose-900/90 to-red-900/90 border border-rose-500/50 rounded-3xl shadow-2xl flex flex-col items-center">
                  ...
                  <p className="text-rose-300 text-sm mb-6 font-mono">
      ...
      <AlertDialog open={showExitDialog} onOpenChange={setShowExitDialog}>
        <AlertDialogContent className="bg-background border-border">
          ...
          <AlertDialogFooter className="flex flex-col sm:flex-row gap-2">
            <Button onClick={handleCancelExit} variant="outline" className="border-border text-muted-foreground hover:bg-card">
              Tiếp tục chơi
            </Button>
            <AlertDialogCancel onClick={handleExitGameWithoutSave} className="bg-card text-foreground hover:bg-muted">
              Thoát không lưu
            </AlertDialogCancel>
      ...
      <AlertDialog open={showNavigationDialog} onOpenChange={setShowNavigationDialog}>
        <AlertDialogContent className="bg-background border-border text-foreground">
          ...
          <AlertDialogFooter className="flex flex-col sm:flex-row gap-2">
            <Button onClick={handleCancelNavigation} variant="outline" className="border-border text-muted-foreground hover:bg-card">
              Ở lại tiếp tục
            </Button>
            <AlertDialogCancel onClick={handleNavigationWithoutSave} className="bg-card text-foreground hover:bg-muted">
              Rời đi không lưu
            </AlertDialogCancel>
...
function ScoreBar({ label, score, target, icon, color }) {
  ...
  return (
    <div className="bg-card border border-border p-3 rounded-2xl backdrop-blur-md">
      ...
      <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
        <div className={cn("h-full bg-gradient-to-r transition-all duration-300", colorClasses[color])} style={{ width: `${percentage}%` }} />
      </div>
    </div>
  );
}
