...
export default function GamePlay() {
  ...
  const [controllerConfig, setControllerConfig] = useState({ disableLeft: false, disableRight: false });

  // Reset config when slug changes
  useEffect(() => {
    setControllerConfig({ disableLeft: false, disableRight: false });
  }, [slug]);

  const renderGameContent = () => {
    const commonProps = { setControllerConfig };

    switch (slug) {
      case "drawing": return <DrawingGame {...commonProps} />;
      ...
      case "caro-5": return <CaroGame winCount={5} {...commonProps} />;
      case "caro-4": return <CaroGame winCount={4} {...commonProps} />;
      case "match-3": return <Match3Game {...commonProps} />;
      default: return (
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 p-6 overflow-hidden">
        ...
        <div className="col-span-4 flex flex-col gap-6 min-h-0">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pr-2 custom-scrollbar">
            <GameController
              disableLeft={controllerConfig.disableLeft}
              disableRight={controllerConfig.disableRight}
            />
          </div>
        </div>
